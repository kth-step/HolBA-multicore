...
(* The extern block relation has two components on the RHS:
 * The local BIR state (bir_state_t)
 * and a polymorphic shared state ('shared_state_t).
...
val _ = Datatype `bir_ext_block_t = <|
  ...
  beb_relation  : 'shared_state_t beb_relation_t |>`;

val _ = Datatype `bir_generic_block_t =
  | BBlock_Stmts ('a  bir_generic_stmt_block_t)
  | BBlock_Ext ('shared_state_t bir_ext_block_t)`;

Datatype:
  bir_generic_program_t = BirProgram ((('a, 'shared_state_t) bir_generic_block_t) list)
End

Type bir_stmt_block_t = `` :bir_stmt_basic_t bir_generic_stmt_block_t``
Type bmc_stmt_block_t = `` :bmc_stmt_basic_t bir_generic_stmt_block_t``

Type bir_block_t = `` :(bir_stmt_basic_t, 'shared_state_t) bir_generic_block_t``
Type bmc_block_t = `` :(bmc_stmt_basic_t, 'shared_state_t) bir_generic_block_t``

Type bir_program_t = ``:(bir_stmt_basic_t, 'shared_state_t) bir_generic_program_t``
Type bmc_program_t = ``:(bmc_stmt_basic_t, 'shared_state_t) bir_generic_program_t``
...
val bir_stmt_ss = rewrites ((type_rws ``:bir_stmt_t``)
                    @ (type_rws ``:bir_stmt_end_t``)
...
Datatype:
  bir_generic_stmt_kind_t =
    | BSGen ('a bir_generic_stmt_t)
    | BSExt ('shared_state_t beb_relation_t)
End
...
val bir_exec_stmtB_def = Define `
  (bir_exec_stmtB (BStmt_Assert ex) st:bir_state_t = bir_exec_stmt_assert ex st) /\
...
val bir_exec_stmt_jmp_def = Define `bir_exec_stmt_jmp p le st =
    case bir_eval_label_exp le st.bst_environ of
      | NONE => bir_state_set_typeerror st
      | SOME l => bir_exec_stmt_jmp_to_label p l st`;

val bir_exec_stmt_cjmp_def = Define `bir_exec_stmt_cjmp p ec l1 l2 st =
  let
...
val bir_exec_stmt_def = Define `
  (bir_exec_stmt p (BStmtB bst) st:bir_state_t =
...
Definition bir_exec_block_ext_def:
  bir_exec_block_ext ext_fun st:bir_state_t =
    (CHOICE (ext_fun (st, EMPTY:bir_state_t -> bool))):bir_state_t
End

Definition bir_exec_step_def:
  bir_exec_step p (state : bir_state_t) =
  if (bir_state_is_terminated state) then state else
    ...
    | SOME $ BSGen stm => bir_exec_stmt p stm state
    | SOME $ BSExt rel => bir_exec_block_ext rel state
End
...
val bir_exec_infinite_steps_fun_def = Define `
  (bir_exec_infinite_steps_fun p st n = FUNPOW (bir_exec_step p) n st)`;
...
``(!p st. (bir_exec_infinite_steps_fun p st 0 = st)) /\
  (!p st n. (bir_exec_infinite_steps_fun p st (SUC n) =
     (bir_exec_infinite_steps_fun p (bir_exec_step p st) n)))``,
  ...
  `(bir_exec_infinite_steps_fun_COUNT_PCs pc_cond p st 0 = 0) /\
   (bir_exec_infinite_steps_fun_COUNT_PCs pc_cond p st (SUC n) =
    let r = bir_exec_infinite_steps_fun_COUNT_PCs pc_cond p
               (bir_exec_step p st) n in
    if (bir_state_COUNT_PC pc_cond (bir_exec_step p st)) then SUC r else r)`
...
val bir_execution_result_ss = rewrites (type_rws ``:bir_execution_result_t``);
...
``!ber. IS_BER_Ended ber <=> (?step_count pc_count final_st.
        ber = BER_Ended step_count pc_count final_st)``,
...
val valOf_BER_Ended_def = Define `valOf_BER_Ended (BER_Ended step_count pc_count final_st) =
  (step_count, pc_count, final_st)`;

val valOf_BER_Ended_steps_def = Define `valOf_BER_Ended_steps (BER_Ended step_count pc_count final_st) =
  (step_count, final_st)`;
