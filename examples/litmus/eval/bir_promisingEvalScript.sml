...
Definition eval_cstep_seq_def:
  ...
  | SOME (BStmtB (BMCStmt_Store var_succ a_e v_e xcl acq rel)) =>
      ...
      MAP (λ(s',msg). (s', [msg])) (eval_cstep_seq_store cid s M var_succ a_e v_e xcl acq rel)
  | SOME (BStmtB (BMCStmt_Fence K1 K2)) =>
...
Definition eval_certify_def:
  ...
  (eval_certify (SUC f) cid p (s,M) =
   ((s.bst_prom = []) ∨ EXISTS (λ(s',ml). eval_certify f cid p (s',M ++ ml)) (eval_cstep_seq cid p (s,M))))
End
...
Definition eval_pfind_def:
  ...
  LIST_BIND (eval_cstep_seq cid p (s,M)) (λ(s',ml). ml ++ eval_pfind f cid p (s', M ++ ml))
End
